...
if (document.getElementById('trp-editor-container')) {
  window.tpEditorApp = new vue__WEBPACK_IMPORTED_MODULE_0___default.a({
    ...
    data: {},
    methods: {
      addToDictionary: function addToDictionary(strings) {
        var extraNodeInfo = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
...
/* harmony default export */ __webpack_exports__["default"] = ({
  props: ['trp_settings', 'available_languages', 'current_language', 'on_screen_language', 'view_as_roles', 'url_to_load', 'string_selectors', 'data_attributes', 'ajax_url', 'editor_nonces', 'string_type_order'],
  data: function data() {
    return {
  ...
  watch: {
    ...
    selectedIndexesArray: function selectedIndexesArray(newSelectedIndexesArray, oldSelectedIndexesArray) {
  ...
  methods: {
    ...
    setupDictionary: function setupDictionary(baseSelector, typeSlug, languageOfIds) {
      var self = this;
      ...
      stringIdsArray = _toConsumableArray(new Set(stringIdsArray)); //grab Regular strings
    ...
    getNodeInfo: function getNodeInfo(node) {
      ...
      selectors.forEach(function (selector) {
        ...
        if (stringId) {
          ...
          var nodeType = node.getAttribute('data-trp-node-type' + nodeAttribute);
          ...
          if (nodeType) entry.type = nodeType;
          if (nodeDescription) entry.nodeDescription = nodeDescription;
          nodeData.push(entry);
        }
      });
      return nodeData;
    ...
    showPencilIcon: function showPencilIcon(target) {
      var self = this;
      var relatedNode, relatedNodeAttr, position, stringSelector, stringId; //for these tag names we need to insert our HTML before the element and not inside of it
      //@TODO: add/research more

      var beforePosition = ['IMG', 'INPUT', 'TEXTAREA']; //if other icons are showing, remove them

      self.removePencilIcon(); //add class to highlight text
      ...
      var editButton = this.iframe.querySelector('trp-span'); //onclick event listener
      //@NOTE: we might need to add separate events for different buttons (the block split stuff)

      editButton.addEventListener('click', function (event) {
        event.preventDefault(); //get node info based on where we inserted our button

        if (position == 'afterbegin') relatedNode = self.iframe.getElementsByTagName('trp-span')[0].parentNode;else relatedNode = self.iframe.getElementsByTagName('trp-span')[0].nextElementSibling;
    ...
    showString: function showString(string, type) {
      if (_typeof(string.attribute) != undefined && (string.attribute == 'href' || string.attribute == 'src')) return false;
      if (string.type == type) return true;
      return false;
    }
  },
          ...
          _c("div", { staticClass: "trp-controls-section-content" }, [
            _c("div", { attrs: { id: "trp-language-switch" } }, [
              _c(
                ...
                _vm._l(_vm.languages, function(lang, langIndex) {
                ...
                _vm._l(_vm.stringTypes, function(type) {
                  return _c(
                    ...
                    _vm._l(_vm.dictionary, function(string, index) {
                      return _vm.showString(string, type)
                        ? _c(
                            "option",
                            {
                              attrs: {
                                title: string.nodeDescription,
            ...
            [
              _c(
                "div",
                {
                  ...
                  attrs: { id: "trp-unsaved-changes-warning-message" }
                },
              ...
              _c(
                "div",
                {
                  staticClass: "trp-language-text trp-default-language",
                  attrs: {
                    id: "trp-language-" + _vm.settings["default-language"]
                  }
                },
                [
                  _c(
                    "div",
                    {
                      staticClass: "trp-language-name",
                      attrs: {
                        "data-trp-gettext-language-name":
                          "To " + _vm.settings["default-language-name"],
                        "data-trp-default-language-name":
                          "From " + _vm.settings["default-language-name"]
                      }
                    },
                    [
                      _vm._v(
                  ...
                  _c("textarea", {
                    attrs: {
                      id: "trp-original",
                      "data-trp-language-code":
                        _vm.settings["default-language"],
                      readonly: "readonly"
                    }
                  }),
                  _vm._v(" "),
                  _c(
                    ...
                    {
                      staticClass:
                        "trp-discard-changes trp-discard-on-default-language",
                      staticStyle: { display: "none" }
                    },
...
var staticRenderFns = [
  ...
  function() {
    ...
    return _c(
      "div",
      {
        staticClass: "trp-language-text trp-gettext-original-language",
        staticStyle: { display: "none" },
        attrs: { id: "trp-gettext-original" }
      },
      [
        _c("div", { staticClass: "trp-language-name" }, [
          _vm._v("Original String")
        ]),
        _vm._v(" "),
        _c("textarea", {
          attrs: { id: "trp-gettext-original-textarea", readonly: "readonly" }
        })
      ]
    )
  }
